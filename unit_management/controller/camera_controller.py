...
async def update_unit(camera_id: int = Path(..., alias="id"), *, updateCamera: CreateCameraModel):
...
async def delete_unit(camera_id: int = Path(..., alias="id")):
    result = await CameraService.delete_by_id(camera_id)
    return result


@router.post("", response_model=ResponseSchema, response_model_exclude_none=True)
async def create_driver(create_data: CreateCameraModel):
